...
namespace UnityEditor.Performance.ProfileAnalyzer
{
    ...
    internal class ProfileData
    {
        ...
        */
        ...
        public void CalcPerFrameThreadTime()
        {
            foreach (var frame in frames)
            {
                frame.CalcPerThreadTime(this);
            }
        }
        ...
        public int GetThreadIndexByThreadName(string threadName)
        {
            int index = -1;
            if (threadNameDict.TryGetValue(threadName, out index))
            {
    ...
    internal class ProfileFrame
    {
        ...
        public void CalcPerThreadTime(ProfileData profileData)
        {
            foreach (var thread in threads)
            {
                thread.CalcThreadTime(profileData);
            }
        }

        public float GetThreadFrameTime(int threadId)
        {
            foreach (var thread in threads)
            {
                if (thread.threadIndex == threadId)
                {
                    return thread.msTime;
                }
            }
    ...
    internal class ProfileThread
    {
        ...
        [NonSerialized]
        public int idleInFrameTimes;
        public ProfileThread()
        {
        }

        //计算每个根的耗时，不计算idle，如果要统计idle的合理性，要另外的工具（比如jobworker，如果出现大量两个idle连在一起，意味着调度是不合理的）。
        public void CalcThreadTime(ProfileData profileData)
        {
            idleInFrameTimes = 0;
            msTime = 0.0f;
            foreach (var marker in markers)
            {
                if (marker.depth == 1)
                {
                    string name = profileData.GetMarkerName(marker);
                    if (name != "Idle")
                    {
                        msTime += marker.msMarkerTotal;
                    }
                    else
                    {
                        idleInFrameTimes++;

...
namespace UnityEditor.Performance.ProfileAnalyzer
{
    ...
    class ProfileTable : TreeView
    {
        ...
        internal static class Styles
        {
            ...
            public static readonly GUIContent menuItemExportMarkerData = new GUIContent("导出该函数单条数据", "");
            public static readonly GUIContent menuItemCopyMeanMinMax = new GUIContent("复制CPU消耗-Mean(Min-Max)", "");
            public static readonly GUIContent menuItemExportMarkerDataBySpecifyPrefix = new GUIContent("根据指定前缀导出该函数下所有数据", "");
        }
            ...
            if (cellRect.Contains(current.mousePosition) && current.type == EventType.ContextClick)
            {
                ...
                */
                ...
                if (content != null && !string.IsNullOrEmpty(content.text))
                {
                    ...
                    menu.AddItem(Styles.menuItemExportMarkerData, false, () => m_ProfileAnalyzerWindow.GetDataFromMarkerNames(new List<string>() { markerName }));
                    menu.AddItem(Styles.menuItemCopyMeanMinMax, false, () =>
                    {
                        string str = m_ProfileAnalyzerWindow.GetDataFromMarkerName(markerName);
                        CopyToClipboard(current, str);
                    });
                    
                    menu.AddItem(Styles.menuItemExportMarkerDataBySpecifyPrefix, false, () => m_ProfileAnalyzerWindow.GetDataFromMarkerNamesBySpecifyPrefixName(new List<string>() { markerName }));
                }

...
namespace UnityEditor.Performance.ProfileAnalyzer
{
    ...
    internal class MarkerData : IComparable<MarkerData>
    {
        ...
        public float GetFrameMsAndCount(int frameIndex,ref int count,ref float selfTime)
        {
            count = 0;
            selfTime = 0.0f;
            foreach (var frameData in frames)
        ...
        public float GetFrameMsInfo(int frameIndex,out int count)
        {
            count = 0;
            foreach (var frameData in frames)
            {
